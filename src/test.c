...
void cl_test()
{
	...
	buffer_t b2 = create_buffer(context, read_write, size);

	fill_buffer(queue, b1, size, a1, true);
	fill_buffer(queue, b2, size, a2, true);

	set_kernel_arg(kernel, b1);
	set_kernel_arg(kernel, b2);

	run_kernel(queue, kernel, 10, 10);

	dump_buffer(queue, b1, size, a1, true);
	flush_queue(queue);
